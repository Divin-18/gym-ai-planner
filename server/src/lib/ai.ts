...
export async function generateTrainingPlan(
  ...
  const daysPerWeek = Number(profile.days_per_week ?? profile.daysPerWeek ?? 4);
  const sessionLength = Number(profile.session_length ?? profile.session ?? 60);

  // Normalize profile data
  const normalizedProfile: UserProfile = {
    goal: profile.goal || "bulk",
    experience: profile.experience || "intermediate",
    days_per_week: Number.isFinite(daysPerWeek) ? daysPerWeek : 4,
    session_length: Number.isFinite(sessionLength) ? sessionLength : 60,
    equipment: profile.equipment || "full_gym",
    injuries: profile.injuries || null,
    preferred_split: profile.preferred_split ?? profile.preferredSplit ?? "upper_lower",
  };
